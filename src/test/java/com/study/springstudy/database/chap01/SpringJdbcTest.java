...
class SpringJdbcTest {
    ...
    void saveTest() {
        // gwt 패턴
        // given : 테스트에 주어질 데이터
        Person p = new Person(1000, "천천히", 3);
    ...
    void modifyTest() {
        //given
        Person person = new Person(66, "칠칠이", 8);
    ...
    @Test
    @DisplayName("사람 정보를 전체조회하면 결과 건수는 4건이며, 첫번째 사람의 이름은 개똥이다.")
    void findAllTest() {
        //given
        // 보내줄 데이터 없으면 생략가능
        ...
        assertEquals("개똥이", people.get(0).getPersonName());
    }
    
    @Test
    @DisplayName("사람 정보를 아이디로 단일조회시 아이디가 100인 사람의 이름은 백이고 나이는 1살이다.")
    void findOneTest() {
        //given
        long id = 100;
        ...
        assertEquals("백", person.getPersonName());
        assertEquals(1, person.getPersonAge());
    }
